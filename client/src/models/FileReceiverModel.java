...
public class FileReceiverModel {
    ...
    public FileReceiverModel(int fileID, Socket socket, EventFileReceiver event) {
        this.fileID = fileID;
        this.socket = socket;
        this.event = event;
    }
    ...
    public void initReceive() {
        System.out.println("TOI BAT DAU EMIT GET FILE");
        socket.emit("get_file", fileID, new Ack() {
            @Override
            public void call(Object... os) {
                if (os.length > 0) {
                    try {
                        System.out.println("TOI VAO EMIT GET FILE ROI NE");
                        ...
                        //  start save file
                        System.out.println("TOI BAT DAU CALL SAVE FILE");
                        startSaveFile();
    ...
    public void startSaveFile() throws IOException, JSONException {
        RequestFileModel data = new RequestFileModel(fileID, accFile.length());
        System.out.println("TOI BAT DAU EMIT REQUEST FILE");
            ...
            public void call(Object... os) {
                try {
                    if (os.length > 0) {
                        ...
                        //  remove list
                        Service.getInstance().fileReceiveFinish(FileReceiverModel.this);
                    }
                } catch (IOException | JSONException e) {
                    e.printStackTrace();
                }
